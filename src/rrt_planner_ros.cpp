...
namespace rrt_planner {
    ...
    RRTPlannerROS::RRTPlannerROS(): costmap_(NULL), planner_(), initialized_(false) {}

    RRTPlannerROS::RRTPlannerROS(std::string name, costmap_2d::Costmap2DROS *costmap): costmap_(NULL), planner_(), initialized_(false) {
        initialize(name, costmap);
    }

    RRTPlannerROS::RRTPlannerROS(std::string name, costmap_2d::Costmap2DROS *costmap, std::string global_frame): costmap_(NULL), planner_(), initialized_(false) {
        initialize(name, costmap, global_frame);
    }

    void RRTPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS *costmap, std::string global_frame) {
        if(!initialized_) {
    ...
    bool RRTPlannerROS::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan) {
      if(!initialized_){
      ...
      if(planner_->planPath()){
        ...
        while (current_id_) {
          ...
          quat_tf_.setRPY(0.0, 0.0, yaw_); // Convert RPY to quat
